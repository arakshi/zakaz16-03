...
class DatasetLoader:
    ...
    def load(self) -> DatasetBundle:
        ...
        df["ai_all"] = df["label"].apply(lambda x: "ai" if x.startswith("ai_") else "human")
    ...
    def build_binary_view(self, df: pd.DataFrame, scenario: str) -> pd.DataFrame:
        if scenario == "human_vs_ai_all":
            out = df[df["label"].isin(["human", "ai_weak", "ai_strong"])].copy()
            out["target"] = out["label"].apply(lambda x: 0 if x == "human" else 1)
            return out
        if scenario == "human_vs_ai_weak":
            out = df[df["label"].isin(["human", "ai_weak"])].copy()
            out["target"] = out["label"].apply(lambda x: 0 if x == "human" else 1)
            return out
        if scenario == "human_vs_ai_strong":
            out = df[df["label"].isin(["human", "ai_strong"])].copy()
            out["target"] = out["label"].apply(lambda x: 0 if x == "human" else 1)
            return out
        ...
        for file_path in class_dir.iterdir():
            ...
            if file_path.suffix.lower() == ".txt":
                ...
                yield {
                    "text": text,
                    "label": label,
                    "source_file": str(file_path.relative_to(self.raw_root.parent)),
                }
                ...
                for idx, row in csv_df.iterrows():
                    yield {
                        "text": str(row["text"]),
                        "label": label,
                        "source_file": f"{file_path.name}#{idx}",
                    }
