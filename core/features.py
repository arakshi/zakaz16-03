...
INTRO_PHRASES = {"возможно", "вероятно", "кажется", "кстати", "однако", "например", "вообще", "безусловно"}
...
class FeatureExtractor:
    ...
    def fit_transform(self, texts: Iterable[str]) -> FeatureSet:
        ...
        word_freq = self._corpus_word_frequency(processed)
        rows = [self._single_features(doc.clean_text, doc.sentences, doc.tokens, word_freq) for doc in processed]
    ...
    def _single_features(self, clean_text: str, sentences: List[str], tokens: List[str], corpus_freq: Counter) -> Dict[str, float]:
        words = [t for t in tokens if t.isalpha()]
        word_count = len(words)
        unique_count = len(set(words))
        sentence_lens = [len([t for t in s.split() if t.strip(string.punctuation)]) for s in sentences] or [0]
        ...
        return {
            ...
            "unique_word_count": float(unique_count),
            ...
            "short_sentence_share": float(np.mean([l <= 7 for l in sentence_lens])) if sentence_lens else 0.0,
            "long_sentence_share": float(np.mean([l >= 20 for l in sentence_lens])) if sentence_lens else 0.0,
            ...
            "punct_density_100w": (sum(punct_counts.values()) / word_count * 100) if word_count else 0.0,
            ...
            "quotes_freq": punct_counts.get("\"", 0) + punct_counts.get("'", 0) + punct_counts.get("«", 0) + punct_counts.get("»", 0),
...
class OptionalPerplexityEstimator:
    """Optional module. Returns NaN if transformers/torch unavailable."""
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self._torch = torch
            self._tokenizer = AutoTokenizer.from_pretrained("sberbank-ai/rugpt3small_based_on_gpt2", local_files_only=True)
            self._model = AutoModelForCausalLM.from_pretrained("sberbank-ai/rugpt3small_based_on_gpt2", local_files_only=True)
            self.available = True
        except Exception:
            self.available = False
