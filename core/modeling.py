...
class TextModelTrainer:
    ...
    def build_models(self) -> Dict[str, ClassifierMixin]:
        return {
            "log_reg": LogisticRegression(max_iter=2000, random_state=self.random_state),
    ...
    def run_experiment(
        ...
        for model_name, model in self.build_models().items():
            ...
            train_df = pd.DataFrame({"text": x_train_txt}).join(x_train_f.reset_index(drop=True))
            test_df = pd.DataFrame({"text": x_test_txt}).join(x_test_f.reset_index(drop=True))
    ...
    def _build_hybrid_pipeline(self, classifier: ClassifierMixin) -> Pipeline:
        ...
        preprocessor = ColumnTransformer(
            transformers=[
                ("text", text_pipe, "text"),
                ("num", numeric_pipe, [
                    "word_count", "unique_word_count", "avg_word_len", "type_token_ratio", "long_word_share",
                    "rare_word_share", "sentence_count", "avg_sentence_len", "std_sentence_len",
                    "short_sentence_share", "long_sentence_share", "service_word_freq", "repeat_ratio",
                    "template_ratio", "intro_phrase_share", "smoothness", "punct_density_100w", "comma_freq",
                    "dot_freq", "colon_freq", "dash_freq", "brackets_freq", "quotes_freq", "question_freq", "exclamation_freq"
                ]),
            ],
            remainder="drop",
        )
