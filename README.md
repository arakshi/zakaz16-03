...
- Поддержка классов: `human`, `ai_deepseek`, `ai_gpt_fast`, `ai_gpt_reasoning`, `ai_yandex` (и совместимость с legacy `ai_weak`/`ai_strong`).
...
- Вкладка загрузки поддерживает: 
  ...
  - кнопка добавления расширенного тестового датасета (human / DeepSeek / GPT Fast / GPT Reasoning / Yandex AI).
