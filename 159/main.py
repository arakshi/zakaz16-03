...
def render_data_upload_tab(service: ProjectService) -> None:
    ...
    st.caption(
        "Можно добавить данные вручную, вставкой текста или загрузкой файлов в нужный класс."
    )
    ...
    with col1:
        ...
        manual_text = st.text_area(
            "Текст для сохранения", height=170, key="manual_text"
        )
    ...
    with col2:
        ...
        upload_label = st.selectbox(
            "Класс для файла", LABEL_OPTIONS, key="upload_label"
        )
        uploaded_file = st.file_uploader(
            "Файл txt/csv", type=["txt", "csv"], key="file_uploader"
        )
        if st.button("Сохранить файл в data/raw", key="save_file"):
            if uploaded_file is None:
                st.warning("Сначала выберите файл.")
            else:
                saved_path = service.save_uploaded_file(
                    uploaded_file.name, uploaded_file.getvalue(), upload_label
                )
    ...
    with st.expander("Примеры промптов для генерации текстов в разных ИИ"):
        st.write(
            "Используйте шаблоны ниже, чтобы быстрее получать сопоставимые тексты в разных моделях."
        )
        topic = st.text_input(
            "Тема для подстановки в шаблон", value="влияние цифровизации на образование"
        )
        ai_target = st.selectbox(
            "Целевой ИИ", ["DeepSeek", "GPT Быстрая", "GPT Думающая", "Яндекс ИИ"]
        )
        ...
        st.caption(
            "Скопируйте, отправьте в выбранный ИИ, затем вставьте ответ выше или загрузите файлом."
        )
    ...
    with c1:
        st.caption(
            "Добавляет расширенный демо-набор в классы human / ai_deepseek / ai_gpt_fast / ai_gpt_reasoning / ai_yandex / artificial_humanized."
        )
    with c2:
...
def main() -> None:
    ...
    with tabs[1]:
        ...
        if st.button("Обучить и оценить"):
            ...
            st.dataframe(
                artifacts.results.sort_values(
                    ["scenario", "f1"], ascending=[True, False]
                )
            )
    ...
    with tabs[3]:
        ...
        example_choice = st.selectbox(
            "Быстрый пример текста", ["(не выбран)", *ANALYSIS_EXAMPLES.keys()]
        )
        initial_text = ANALYSIS_EXAMPLES.get(example_choice, "")
        text = st.text_area("Вставьте текст", value=initial_text)
        st.caption(
            "Совет: для корректной вероятности AI сначала обучите модель на сбалансированном наборе классов."
        )
