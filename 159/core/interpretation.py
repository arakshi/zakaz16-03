...
def extract_feature_importance(model, top_k: int = 15) -> pd.DataFrame:
    ...
    feature_names = ["text_tfidf_features", "numeric_features"]
    num_names = pre.transformers_[1][2]

    if hasattr(clf, "coef_"):
        coefs = clf.coef_[0]
        names = [f"tfidf_{i}" for i in range(len(coefs) - len(num_names))] + list(num_names)
    ...
    if hasattr(clf, "feature_importances_"):
        ...
        names = [f"tfidf_{i}" for i in range(len(importances) - len(num_names))] + list(num_names)
        imp = pd.DataFrame({"feature": names, "importance": importances})
        return imp.sort_values("importance", ascending=False).head(top_k)

    return pd.DataFrame({"feature": feature_names, "importance": [np.nan, np.nan]})
