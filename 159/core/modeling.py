...
class TextModelTrainer:
    ...
    def run_experiment(
        ...
        # For very small class counts fallback to train==test instead of crashing.
        if label_counts.min() < 2:
            x_train_f, x_test_f = features.copy(), features.copy()
            y_train, y_test = labels.copy(), labels.copy()
            x_train_txt, x_test_txt = text_series.copy(), text_series.copy()
        else:
            ...
            if y_train.nunique() < 2 or y_test.nunique() < 2:
                # fallback for unlucky split on tiny datasets
                x_train_f, x_test_f = features.copy(), features.copy()
                y_train, y_test = labels.copy(), labels.copy()
                x_train_txt, x_test_txt = text_series.copy(), text_series.copy()
